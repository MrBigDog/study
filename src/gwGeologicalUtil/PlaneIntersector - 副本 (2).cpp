...
namespace gwUtil
{
	namespace PlaneIntersectorUtils
	{
		...
		struct TriangleIntersector
		{
			...
			{
				//_polylineConnector1._isCanLoop = false;
			}
		...
		void mergeLine(PolylineConnector::PolylineList& polylist, /*const*/ PolylineConnector::PolylineList& additionalPl)
		{
			...
			//if (additionalPl.empty() || polylist.empty())
			//{
			//	return;
			//}

			//unsigned int adSize = additionalPl.size();

			//PolylineConnector::PolylineList::iterator ait;
			...
			//	osg::ref_ptr<RefPolyline> apolylien = *ait;
			...
			//		{
			//			RefPolyline* pl = *pit;
			//			if (pl->_polyline.size() < 2) continue;
			...
			//	if (junctionVec.empty())
			//	{
			//		ait++;
			//		continue;
			//	}

			//	ait = additionalPl.erase(ait);
			...
			//		}
			//	}
			//	if (junctionVec.size() == 1)
			//	{
			//		if (junctionVec[0]->_index == apolylien->_polyline.size() - 1)
			//		{
			//			Junction* junction = junctionVec[0];
			//			Junction::PolylineVec::const_iterator pit;
			//			for (pit = junction->_polylines.begin(); pit != junction->_polylines.end(); ++pit)
			//			{
			//				unsigned int polylineInd = pit->first;
			//				if (polylineInd == 0)
			//				{
			//					for (int i = apolylien->_polyline.size() - 2; i >= 0; --i)
			//					{
			//						pit->second->_polyline.insert(pit->second->_polyline.begin(), apolylien->_polyline.at(i));
			//					}
			//				}
			...
			//	}
			//}
			//if (additionalPl.size() != adSize)
			//{
			//	mergeLine(polylist, additionalPl);
			//}
		}
	}
	...
	unsigned int PlaneIntersector::getSize() const
	{
		return _mysize;
	}
