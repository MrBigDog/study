...
namespace cut
{
	...
	void Mesh::cut(Mesh* left, Mesh* right, const osg::Vec3d& planePoint, const osg::Vec3d& planeNormal)
	{
		...
		left->_va = new osg::Vec3Array(); left->_va->reserve(newVertexCount);

		//left->vertexNormals = new Vector3[newVertexCount];
		left->_na = new osg::Vec3Array(); left->_na->reserve(newVertexCount);

		//left->indices = new int[leftIndexCount];
		left->_ia = new osg::UIntArray(); left->_ia->reserve(leftIndexCount);
