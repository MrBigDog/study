...
const double elipse = DBL_EPSILON;
	...
	namespace PlaneIntersectorUtils
	{
				...
				else
				{
					...
					if (v1_connected_to_start)
					{
						...
						else if (v2_connected_to_end)
						{
							...
							{
								//insertAtStart(v2, v1_start_itr);//生成闭环
								RefPolyline* polyline = v1_start_itr->second.get();
					...
					else if (v1_connected_to_end)
					{
						if (v2_connected_to_start)
						{
							...
							{
								//insertAtEnd(v2, v1_end_itr);//生成闭环
								RefPolyline* polyline = v1_end_itr->second.get();
		...
		struct TriangleIntersector
		{
			...
			bool isContain(const osg::Vec4d& v1, const osg::Vec4d& v2, const IntersectedEdgeVector& intersectedEdges)
			{
				...
				{
					if (isVec4dEqual(it->_vs, v1, DBL_EPSILON) && isVec4dEqual(it->_ve, v2, DBL_EPSILON) ||
						isVec4dEqual(it->_vs, v2, DBL_EPSILON) && isVec4dEqual(it->_ve, v1, DBL_EPSILON))
					{
						return true;
					}
				}
