	...
	namespace PlaneIntersectorUtils
	{
		...
		void mergeLine(PolylineConnector::PolylineList& polylist, const PolylineConnector::PolylineList& additionalPl)
		{
			...
			for (ait = additionalPl.begin(); ait != additionalPl.end(); ++ait)
			{
				...
				{
					osg::Vec4d v = *vait;
					osg::ref_ptr<Junction> junction = new Junction(vait - apolylien->_polyline.begin());
				...
				{
					unsigned int currentIndex = jit->get()->_index;
					Junction* junction = *jit;

					RefPolyline::Polyline subset;
					subset.reserve(lastIndex - currentIndex + 1);
					for (unsigned int i = currentIndex; i <= lastIndex; ++i)
					{
						subset.push_back(apolylien->_polyline.at(i));
					}
					...
					for (pit = junction->_polylines.begin(); pit != junction->_polylines.end(); ++pit)
					{
						...
						if (polylineInd == 0)
						{
							pit->second->_polyline.insert(pit->second->_polyline.begin(), subset.begin(), subset.end());
						}
						else
						{
							pit->second->_polyline.insert(pit->second->_polyline.end(), subset.begin(), subset.end());
						}
					}
				}
				...
				//{
				//	RefPolyline* pl = *pit;
				//	std::vector<std::pair<unsigned int, osg::ref_ptr<Junction> > > indexVec;
				//	for (JunctionVector::const_iterator jit = junctionVec.begin(); jit != junctionVec.end(); ++jit)
				//	{
				//		for (Junction::PolylineVec::const_iterator it = (*jit)->_polylines.begin(); it != (*jit)->_polylines.end(); ++it)
				//		{
				//			RefPolyline* p = (*it).second;
				//			if (pl == p) indexVec.push_back(std::pair<unsigned int, osg::ref_ptr<Junction>>(it->first, *jit));
				//		}
				//	}
				//	if (indexVec.size() > 1)
				//	{
				//		//闭环;
				//		unsigned int juncInd1 = indexVec[0].second->_index;
				//		unsigned int juncInd2 = indexVec[1].second->_index;
				//		unsigned int polyInd1 = indexVec[0].first;
				//		unsigned int polyInd2 = indexVec[1].first;
				//		if (juncInd1 == juncInd2 || polyInd1 == polyInd2)
				//		{
				//			continue;
				//		}
				//		unsigned int firstInd = polyInd1 < polyInd2 ? juncInd2 : juncInd1;
				//		RefPolyline::Polyline subset;
				//		if (polyInd1 < polyInd2)
				//		{
				//			getSubset(apolylien->_polyline, juncInd2, juncInd1, subset);
				//		}
				//		else
				//		{
				//			getSubset(apolylien->_polyline, juncInd1, juncInd2, subset);
				//		}
				//		if (subset.empty())
				//		{
				//			continue;
				//		}
				//		for (RefPolyline::Polyline::const_iterator subIt = subset.begin(); subIt != subset.end(); ++subIt)
				//		{
				//			apolylien->_polyline.push_back(*subIt);
				//		}
				//		//删除相应的Polyline;
				//	}
				//}
