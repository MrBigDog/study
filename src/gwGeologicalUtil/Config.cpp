...
void Config::setReferrer(const std::string& referrer)
{
	if (referrer.empty())
		return;
	...
	if (!osgDB::containsServerAddress(referrer) && !osgDB::isAbsolutePath(referrer)) {
		...
		absReferrer = gwUtil::getAbsolutePath(referrer);

		if (gwUtil::isRelativePath(absReferrer))
		{
			GW_WARN << LC << "ILLEGAL: call to setReferrer with relative path:  "
...
bool Config::fromXML(std::istream& in)
{
...
Config Config::child(const std::string& childName) const
{
...
const Config* Config::child_ptr(const std::string& childName) const
{
	for (ConfigSet::const_iterator i = _children.begin(); i != _children.end(); i++) {
		if (i->key() == childName)
			return &(*i);
...
Config* Config::mutable_child(const std::string& childName)
{
	for (ConfigSet::iterator i = _children.begin(); i != _children.end(); i++) {
		if (i->key() == childName)
			return &(*i);
...
const Config* Config::find(const std::string& key, bool checkMe) const
{
...
Config* Config::find(const std::string& key, bool checkMe)
{
	if (checkMe && key == this->key())
		return this;
...
bool Config::fromJSON(const std::string& input)
{
	Json::Reader reader;
...
Config Config::readJSON(const std::string& json)
{
	Config conf;
	conf.fromJSON(json);
	return conf;
