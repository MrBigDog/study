...
namespace cut
{
	...
	//----------------------------------------------------------------------------------------------------------------------------
	Mesh::Mesh() : vertexCount(0), indexCount(0), vertices(nullptr), indices(nullptr), vertexNormals(nullptr), texCoords(nullptr)
	{}
	...
	void Mesh::fromGeometry(osg::Geometry * geom)
	{
		if (!geom) return;
	...
	void Mesh::cut(Mesh* left, Mesh* right, const osg::Vec3d& planePoint, const osg::Vec3d& planeNormal)
	{
		int faceCount = _ia->size() / 3;

		//// The maximum number of vertices for the new meshes is vertexCount + faceCount * 2
		//// (two points of intersection for each face)
		vertexCount = _va->size();
		...
		osg::ref_ptr<osg::Vec3Array> newVertices = new osg::Vec3Array; newVertices->reserve(newVertexMax);
		for (osg::Vec3Array::const_iterator it = _va->begin(); it != _va->end(); ++it)
		{
		...
		osg::ref_ptr<osg::Vec3Array> newNormals = new osg::Vec3Array; newNormals->reserve(newVertexMax);
		for (osg::Vec3Array::const_iterator it = _na->begin(); it != _na->end(); ++it)
		{
			newNormals->push_back(*it);
		}
		...
		osg::ref_ptr<osg::UIntArray> leftIndices = new osg::UIntArray(); leftIndices->resize(newIndexMax);

		int rightIndexCount = 0;
		//int* rightIndices = new int[newIndexMax];
		osg::ref_ptr<osg::UIntArray> rightIndices = new osg::UIntArray(); rightIndices->resize(newIndexMax);

		// Iterate through each face and decide which list to put it in and whether to divide it
		for (int i = 0; i < faceCount; ++i)
		{
			int i1 = _ia->at(i * 3 + 0);//indices[i * 3 + 0];
			int i2 = _ia->at(i * 3 + 1);//indices[i * 3 + 1];
			int i3 = _ia->at(i * 3 + 2);//indices[i * 3 + 2];

			osg::Vec3 v1 = _va->at(i1);//Vector3* v1 = &vertices[i1];
			osg::Vec3 v2 = _va->at(i2);//Vector3* v2 = &vertices[i2];
			osg::Vec3 v3 = _va->at(i3);//Vector3* v3 = &vertices[i3];

			// Check if each vertex is to the left of the plane
			osg::Vec3 v1ToPlane; sub3(planePoint, v1, v1ToPlane);
			osg::Vec3 v2ToPlane; sub3(planePoint, v2, v2ToPlane);
			osg::Vec3 v3ToPlane; sub3(planePoint, v3, v3ToPlane);
			...
			if (pointsToLeft == 3)
			{
				...
				/*leftIndices->push_back(i1);//*/(*leftIndices)[leftIndexCount++] = i1;
				/*leftIndices->push_back(i2);//*/(*leftIndices)[leftIndexCount++] = i2;
				/*leftIndices->push_back(i3);//*/(*leftIndices)[leftIndexCount++] = i3;
			}
			// All vertices to right
			else if (pointsToLeft == 0)
			{
				// Add all vertices to right
				/*rightIndices->push_back(i1);//*/(*rightIndices)[rightIndexCount++] = i1;
				/*rightIndices->push_back(i2);//*/(*rightIndices)[rightIndexCount++] = i2;
				/*rightIndices->push_back(i3);//*/(*rightIndices)[rightIndexCount++] = i3;
			}
			// One vertex to left
			else if (pointsToLeft == 1)
			{
				if (v1Left)
				{
					...
					const osg::Vec3 a = (*newVertices)[ia];
					const osg::Vec3 b = (*newVertices)[ib];
					const osg::Vec3 c = (*newVertices)[ic];
					...
					lerp3((*_na)[ia], (*_na)[ib], intersect1coeff, (*newNormals)[intersect1index]);
					lerp3((*_na)[ia], (*_na)[ic], intersect2coeff, (*newNormals)[intersect2index]);

					// Add triangles
					// Left
					/*leftIndices->push_back(i1);             //*/(*leftIndices)[leftIndexCount++] = i1;
					/*leftIndices->push_back(intersect1index);//*/(*leftIndices)[leftIndexCount++] = intersect1index;
					/*leftIndices->push_back(intersect2index);//*/(*leftIndices)[leftIndexCount++] = intersect2index;

					// Right
					/*rightIndices->push_back(intersect1index);//*/(*rightIndices)[rightIndexCount++] = intersect1index;
					/*rightIndices->push_back(i2);             //*/(*rightIndices)[rightIndexCount++] = i2;
					/*rightIndices->push_back(i3);             //*/(*rightIndices)[rightIndexCount++] = i3;
					/*										   */				 
					/*rightIndices->push_back(intersect1index);//*/(*rightIndices)[rightIndexCount++] = intersect1index;
					/*rightIndices->push_back(i3);             //*/(*rightIndices)[rightIndexCount++] = i3;
					/*rightIndices->push_back(intersect2index);//*/(*rightIndices)[rightIndexCount++] = intersect2index;
				}
				else if (v2Left)
				{
					osg::Vec3 line1, line2;
