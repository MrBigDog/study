...
std::string gwUtil::getAbsolutePath(const std::string& path)
{
	return osgDB::convertFileNameToUnixStyle(osgDB::getRealPath(path));
}

bool gwUtil::isRelativePath(const std::string& fileName)
{
...
std::string gwUtil::getFullPath(const std::string& relativeTo, const std::string &relativePath)
{
...
bool gwUtil::isArchive(const std::string& path)
{
	...
	{
		if (gwUtil::endsWith(path, ("." + *i), false))
			return true;
	}
	return false;
}

bool gwUtil::isPathToArchivedFile(const std::string& path)
{
	osgDB::Registry::ArchiveExtensionList list = osgDB::Registry::instance()->getArchiveExtensions();
	for (osgDB::Registry::ArchiveExtensionList::const_iterator i = list.begin(); i != list.end(); ++i)
	{
...
bool gwUtil::isZipPath(const std::string &path)
{
	GW_WARN << LC << "FileUtils::isZipPath is deprecated; use isPathToArchivedFile instead" << std::endl;
	return (path.find(".zip") != std::string::npos);
}

std::string gwUtil::getTempPath()
{
#if defined(WIN32)  && !defined(__CYGWIN__)
	BOOL fSuccess = FALSE;
	...
	{
		GW_NOTICE << "GetTempPath failed" << std::endl;
		return ".";
	}
...
std::string gwUtil::getTempName(const std::string& prefix, const std::string& suffix)
{
...
bool gwUtil::makeDirectory(const std::string &path)
{
	if (path.empty())
	{
		GW_DEBUG << "osgDB::makeDirectory(): cannot create an empty directory" << std::endl;
		return false;
	}
	...
	while (true)
	{
...
#endif
		{
			...
			default:
				GW_DEBUG << "osgDB::makeDirectory(): " << strerror(errno) << std::endl;
				return false;
			}
		}
	...
	while (!paths.empty())
	{
...
#endif
		{
			...
			{
				GW_DEBUG << "Attempt to create directory that already exists " << dir << std::endl;
			}
			else
			{
				GW_DEBUG << "osgDB::makeDirectory(): " << strerror(errno) << std::endl;
...
bool gwUtil::touchFile(const std::string& path)
{
	DateTime now;
	struct ::utimbuf ut;
...
TimeStamp gwUtil::getLastModifiedTime(const std::string& path)
{
	struct stat buf;
