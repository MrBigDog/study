...
XmlElement::getSubElement(const std::string& name) const
{
	for (XmlNodeList::const_iterator i = getChildren().begin(); i != getChildren().end(); i++)
	{
		if (i->get()->isElement())
		{
			...
			if (gwUtil::ciEquals(name, e->getName()))
				return e;
		}
	}
	return NULL;
...
Config XmlElement::getConfig(const std::string& referrer) const
{
	if (isInclude())
	{
		...
		if (href.empty())
		{
			GW_WARN << "Missing href with xi:include" << std::endl;
			return Config();
		}
		...
		OSG_INFO << "Loading href from " << fullURI << std::endl;
		...
		{
			GW_WARN << "Failed to load xi:include from " << fullURI << std::endl;
			return Config();
		}
	}
	else
...
namespace
{
	...
	void processNode(XmlElement* parent, TiXmlNode* node)
	{
		...
		case TiXmlNode::TINYXML_ELEMENT:
		{
			...
			std::string tag = gwUtil::toLower(element->Value());
			...
			while (attr)
			{
				std::string name = gwUtil::toLower(attr->Name());
...
XmlDocument::load(std::istream& in, const URIContext& uriContext)
{
	...
	if (xmlDoc.Error())
	{
		...
		std::string str;
		str = buf.str();
		GW_WARN << "Error in XML document: " << str << std::endl;
		if (!uriContext.referrer().empty())
			GW_WARN << uriContext.referrer() << std::endl;
	}
