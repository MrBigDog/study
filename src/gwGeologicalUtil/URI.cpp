...
{
	return gwUtil::getFullPath(_referrer, target);
}

URIContext
URIContext::add(const URIContext& sub) const
{
	return URIContext(gwUtil::getFullPath(_referrer, sub._referrer));
}
...
URIContext::store(osgDB::Options* options)
{
	if (options)
	{
		if (_referrer.empty() == false)
		{
			...
			options->setPluginStringData("gwUtil::URIContext::referrer", _referrer);
		}
	}
}
...
URI::ctorCacheKey()
{
	std::string temp = Stringify() << std::hex << std::setw(8) << std::setfill('0') << gwUtil::hashString(_fullURI);
	_cacheKey
...
namespace
{
	// convert an osgDB::ReaderWriter::ReadResult to an gwUtil::ReadResult
		...
		for (osgDB::Registry::ArchiveExtensionList::iterator aitr = e.begin(); aitr != e.end(); ++aitr)
		{
			...
			if (positionArchive != std::string::npos)
			{
				...
				if (rr.success())
				{
					...
					result.setLastModifiedTime(gwUtil::getLastModifiedTime(uri));
					return result;
				}
				else
				{
					...
					if (result.empty() || expired)
					{
						...
						if (!gotResultFromCallback)
						{
							...
							if ((result.empty() || expired) && cp->usage() != CachePolicy::USAGE_CACHE_ONLY)
							{
								...
								{
									GW_DEBUG << LC << uri.full() << " not modified, using cached result" << std::endl;
								...
								{
									GW_DEBUG << LC << "Got remote result for " << uri.full() << std::endl;
									result = remoteResult;
								}
							}
							...
							{
								GW_DEBUG << LC << "Writing " << uri.cacheKey() << " to cache" << std::endl;
...
{
	OSG_INFO << LC << "Map: " << filename << " to " << _aliasMap.resolve(filename, _context) << std::endl;
