...
namespace gwUtil
{
	namespace PlaneIntersectorUtils
	{
		...
		public:
			...
			PolylineConnector() :_isCanLoop(true) {}
		...
		{
			for (PolylineConnector::PolylineList::const_iterator it = additionalPl.begin(); it != additionalPl.end(); ++it)
			{
				polylist.push_back(*it);
			}
			return;
		}
	}
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//HullPlaneIntersector
	HullPlaneIntersector::HullPlaneIntersector(const osg::Plane& plane, const osg::Polytope& boundingPolytope)
		: _parent(0)
		, _recordHeightsAsAttributes(false)
		, _plane(plane)
	...
	HullPlaneIntersector::HullPlaneIntersector(CoordinateFrame cf, const osg::Plane& plane, const osg::Polytope& boundingPolytope)
		: Intersector(cf)
		, _parent(0)
		, _recordHeightsAsAttributes(false)
		, _plane(plane)
	...
	osgUtil::Intersector* HullPlaneIntersector::clone(osgUtil::IntersectionVisitor& iv)
	{
		if (_coordinateFrame == MODEL && iv.getModelMatrix() == 0)
		{
			osg::ref_ptr<HullPlaneIntersector> pi = new HullPlaneIntersector(_plane, _polytope);
			pi->_parent = this;
		...
		osg::ref_ptr<HullPlaneIntersector> pi = new HullPlaneIntersector(plane, transformedPolytope);
		pi->_parent = this;
		pi->_intersectionLimit = this->_intersectionLimit;
		pi->_recordHeightsAsAttributes = _recordHeightsAsAttributes;
		pi->_em = _em;
	...
	bool HullPlaneIntersector::enter(const osg::Node& node)
	{
	...
	void HullPlaneIntersector::leave()
	{
		// do nothing.
	}

	unsigned int HullPlaneIntersector::getSize() const
	{
		return _mysize;
	}

	void HullPlaneIntersector::intersect(osgUtil::IntersectionVisitor& iv, osg::Drawable* drawable)
	{
		...
		_mysize = ti._polylineConnector._polylines.size();
	...
	void HullPlaneIntersector::reset()
	{
		Intersector::reset();
