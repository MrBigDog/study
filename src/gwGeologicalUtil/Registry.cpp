...
void Registry::setDefaultCachePolicy(const CachePolicy& value)
{
	_defaultCachePolicy = value;
}

void Registry::setOverrideCachePolicy(const CachePolicy& value)
{
	_overrideCachePolicy = value;
}

bool Registry::resolveCachePolicy(optional<CachePolicy>& cp) const
{
...
const std::string& Registry::getDefaultCacheDriverName() const
{
	if (!_cacheDriver.isSet())
	{
		...
		if (!_cacheDriver.isSet())
		{
			...
			{
				_cacheDriver = value;
				GW_DEBUG << LC << "Cache driver set from environment: " << value << std::endl;
			}
		}
	}
...
const optional<CachePolicy>& Registry::defaultCachePolicy() const
{
	return _defaultCachePolicy;
}

const optional<CachePolicy>& Registry::overrideCachePolicy() const
{
	if (!_overrideCachePolicyInitialized)
	{
		...
		if (!_overrideCachePolicyInitialized)
		{
			...
			if (::getenv(OSGEARTH_ENV_NO_CACHE))
			{
				...
				OSG_INFO << LC << "NO-CACHE MODE set from environment" << std::endl;
			}
			else
			{
				...
				if (::getenv(OSGEARTH_ENV_CACHE_ONLY))
				{
					...
					OSG_INFO << LC << "CACHE-ONLY MODE set from environment" << std::endl;
				}
				...
				if (cacheMaxAge)
				{
					TimeSpan maxAge = gwUtil::as<long>(std::string(cacheMaxAge), INT_MAX);
...
gwUtil::Cache* Registry::getDefaultCache() const
{
...
void Registry::setDefaultCache(Cache* cache)
{
	_defaultCache = cache;
}
...
void Registry::blacklist(const std::string& filename)
{
	...
	GW_DEBUG << "Blacklist size = " << _blacklistedFilenames.size() << std::endl;
}
...
//bool Registry::hasCapabilities() const
//{
//	return _caps.valid();
//}
//
...
//}
//
//void Registry::setCapabilities(Capabilities* caps)
//{
//	_caps = caps;
//}

//void Registry::initCapabilities()
//{
...
//const ShaderFactory* Registry::getShaderFactory() const
//{
...
//ShaderGeneratorProxy Registry::getShaderGenerator() const
//{
...
void Registry::setURIReadCallback(URIReadCallback* callback)
{
	_uriReadCallback = callback;
}

URIReadCallback* Registry::getURIReadCallback() const
{
	return _uriReadCallback.get();
}

void Registry::setDefaultFont(osgText::Font* font)
{
	Threading::ScopedMutexLock exclusive(_regMutex);
	_defaultFont = font;
}

osgText::Font* Registry::getDefaultFont()
{
...
const osgDB::Options* Registry::getDefaultOptions() const
{
	return _defaultOptions.get();
}

osgDB::Options* Registry::cloneOrCreateOptions(const osgDB::Options* input)
{
...
//void Registry::registerUnits(const Units* units)
//{
//	Threading::ScopedWriteLock exclusive(_unitsVectorMutex);
//	_unitsVector.push_back(units);
//}
//
//const Units* Registry::getUnits(const std::string& name) const
//{
...
//void Registry::setDefaultTerrainEngineDriverName(const std::string& name)
//{
//	_terrainEngineDriver = name;
//}

//void Registry::setDefaultCacheDriverName(const std::string& name)
//{
//	_cacheDriver = name;
//}

//void Registry::setStateSetCache(StateSetCache* cache)
//{
//	_stateSetCache = cache;
//}
//
//StateSetCache* Registry::getStateSetCache() const
//{
//	return _stateSetCache.get();
//}

//ProgramSharedRepo* Registry::getProgramSharedRepo()
//{
//	return &_programRepo;
//}
//
//ObjectIndex* Registry::getObjectIndex() const
//{
//	return _objectIndex.get();
//}

//void Registry::startActivity(const std::string& activity)
//{
...
//}
//
//void Registry::endActivity(const std::string& activity)
//{
//	Threading::ScopedMutexLock lock(_activityMutex);
//	_activities.erase(Activity(activity, std::string()));
//}
//
//void Registry::getActivities(std::set<std::string>& output)
//{
//	Threading::ScopedMutexLock lock(_activityMutex);
...
//std::string Registry::getExtensionForMimeType(const std::string& mt)
//{
...
//std::string Registry::getMimeTypeForExtension(const std::string& ext)
//{
...
//void Registry::setTextureImageUnitOffLimits(int unit)
//{
//	Threading::ScopedMutexLock exclusive(_regMutex);
//	_offLimitsTextureImageUnits.insert(unit);
//}
//
//const std::set<int> Registry::getOffLimitsTextureImageUnits() const
//{
//	Threading::ScopedMutexLock exclusive(_regMutex);
