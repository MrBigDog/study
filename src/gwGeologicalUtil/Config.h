#ifndef GWGEOLOGICALUTIL_CONFIG_H
#define GWGEOLOGICALUTIL_CONFIG_H 1
...
namespace gwUtil
{
	 ...
	 */
	class GWGEOLOGICALUTIL_EXPORT Config
	{
	public:
		Config() : _isLocation(false) { }
		...
		void addIfSet(const std::string& key, const optional<T>& opt)
		{
			if (opt.isSet())
			{
				add(key, gwUtil::toString<T>(opt.value()));
			}
		}
		...
		T value(const std::string& key, T fallback) const {
			...
			if (hasChild(key))
				r = child(key).value();
			return gwUtil::as<T>(r, fallback);
		}

		/** Value case to a boolean */
		bool boolValue(bool fallback) const {
			return gwUtil::as<bool>(_defaultValue, fallback);
		}
		...
		bool getIfSet(const std::string& key, optional<T>& output) const {
			...
			if (hasChild(key))
				r = child(key).value();
			if (!r.empty()) {
				output = gwUtil::as<T>(r, output.defaultValue());
				return true;
			}
			else
				return false;
	 ...
	 */
	class GWGEOLOGICALUTIL_EXPORT ConfigOptions
	{
	public:
	 ...
	 */
	class GWGEOLOGICALUTIL_EXPORT DriverConfigOptions : public ConfigOptions
	{
	public:
