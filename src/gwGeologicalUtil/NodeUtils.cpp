...
void PagedLODWithNodeOperations::runPostMerge(osg::Node* node)
{
...
bool PagedLODWithNodeOperations::addChild(osg::Node* child)
{
	bool ok = false;
	if (child)
	{
...
bool PagedLODWithNodeOperations::insertChild(unsigned index, Node* child)
{
	bool ok = false;
	if (child)
	{
...
bool PagedLODWithNodeOperations::replaceChild(Node* origChild, Node* newChild)
{
	bool ok = false;
...
ObserverGroup::traverse(osg::NodeVisitor& nv)
{
	...
	else if (nv.getVisitorType() == nv.UPDATE_VISITOR && _orphans.size() > 0)
	{
		...
		GW_DEBUG << LC << _orphans.size() << " orphaned children removed." << std::endl;
		_orphans.clear();
	}
