...
void ProxySettings::mergeConfig(const Config& conf)
{
...
Config ProxySettings::getConfig() const
{
	Config conf("proxy");
...
bool ProxySettings::fromOptions(const osgDB::Options* dbOptions, optional<ProxySettings>& out)
{
	if (dbOptions)
	{
		std::string jsonString = dbOptions->getPluginStringData("gwUtil::ProxySettings");
		if (!jsonString.empty())
		{
			Config conf;
...
void ProxySettings::apply(osgDB::Options* dbOptions) const
{
	if (dbOptions)
	{
		Config conf = getConfig();
		dbOptions->setPluginStringData("gwUtil::ProxySettings", conf.toJSON());
	}
}
...
namespace gwUtil
{
	...
	static size_t StreamObjectHeaderCallback(void* ptr, size_t size, size_t nmemb, void* data)
	{
		size_t realsize = size* nmemb;
		StreamObject* sp = (StreamObject*)data;
	...
	TimeStamp getCurlFileTime(void* curl)
	{
		long filetime;
...
void HTTPRequest::addParameter(const std::string& name, int value)
{
...
void HTTPRequest::addParameter(const std::string& name, double value)
{
	std::stringstream buf;
	buf << value;
	std::string bufStr;
...
unsigned HTTPResponse::getCode() const
{
...
bool HTTPResponse::isCancelled() const
{
	return _cancelled;
}

unsigned int HTTPResponse::getNumParts() const
{
	return _parts.size();
}

unsigned int HTTPResponse::getPartSize(unsigned int n) const
{
	return _parts[n]->_size;
}

const std::string& HTTPResponse::getPartHeader(unsigned int n, const std::string& name) const
{
...
std::string HTTPResponse::getPartAsString(unsigned int n) const
{
...
Config HTTPResponse::getHeadersAsConfig() const
{
	Config conf;
	if (_parts.size() > 0)
	{
...
void HTTPClient::initialize() const
{
	if (!_initialized)
	{
...
void HTTPClient::initializeImpl()
{
	...
	if (simCode)
	{
		_simResponseCode = gwUtil::as<long>(std::string(simCode), 404L);
		GW_WARN << LC << "Simulating a network error with Response Code = " << _simResponseCode << std::endl;
	}
	...
	{
		s_HTTP_DEBUG = true;
		GW_WARN << LC << "HTTP debugging enabled" << std::endl;
	}

	GW_DEBUG << LC << "HTTPClient setting userAgent=" << userAgent << std::endl;

	curl_easy_setopt(_curl_handle, CURLOPT_USERAGENT, userAgent.c_str());
	curl_easy_setopt(_curl_handle, CURLOPT_WRITEFUNCTION, gwUtil::StreamObjectReadCallback);
	curl_easy_setopt(_curl_handle, CURLOPT_HEADERFUNCTION, gwUtil::StreamObjectHeaderCallback);
	...
	if (timeoutEnv)
	{
		timeout = gwUtil::as<long>(std::string(timeoutEnv), 0);
	}
	GW_DEBUG << LC << "Setting timeout to " << timeout << std::endl;
	...
	{
		connectTimeout = gwUtil::as<long>(std::string(connectTimeoutEnv), 0);
	}
	GW_DEBUG << LC << "Setting connect timeout to " << connectTimeout << std::endl;
...
void HTTPClient::setProxySettings(const ProxySettings& proxySettings)
{
...
void HTTPClient::readOptions(const osgDB::Options* options, std::string& proxy_host, std::string& proxy_port) const
{
...
bool HTTPClient::decodeMultipartStream(const std::string&   boundary,
...
ReadResult HTTPClient::readImage(const HTTPRequest&    request,
	const osgDB::Options* options,
	ProgressCallback*     progress)
{
...
ReadResult HTTPClient::readNode(const HTTPRequest&    request,
	const osgDB::Options* options,
	ProgressCallback*     progress)
{
...
ReadResult HTTPClient::readObject(const HTTPRequest&    request,
	const osgDB::Options* options,
	ProgressCallback*     progress)
{
...
ReadResult HTTPClient::readString(const HTTPRequest&    request,
	const osgDB::Options* options,
	ProgressCallback*     progress)
{
	...
	ProgressCallback*     progress) const
{
	...
	if (rewriter.valid())
	{
		...
		GW_DEBUG << LC << "Rewrote URL " << oldURL << " to " << url << std::endl;
	}
	...
	if (!hInternet)
	{
		GW_WARN << LC << "InternetOpen failed: " << GetLastErrorAsString() << std::endl;
	...
	{
		GW_WARN << LC << "InternetCrackUrl failed for " << url << ": " << GetLastErrorAsString() << std::endl;
	...
	if (!hConnection)
	{
		GW_WARN << LC << "InternetConnect failed for " << url << ": " << GetLastErrorAsString() << std::endl;
		InternetCloseHandle(hInternet);
		return HTTPResponse(0);
	}
	...
	if (!hRequest)
	{
		GW_WARN << LC << "HttpOpenRequest failed for " << url << ": " << GetLastErrorAsString() << std::endl;
	...
	while (!HttpSendRequest(hRequest, NULL, 0, NULL, 0))
	{
		...
		{
			GW_WARN << LC << "Server reports ERROR_INTERNET_CLIENT_AUTH_CERT_NEEDED!\n";
		...
		{
			GW_WARN << LC << "HttpSendRequest failed to open " << url << ": " << GetLastErrorAsString() << std::endl;
			InternetCloseHandle(hRequest);
			InternetCloseHandle(hConnection);
			InternetCloseHandle(hInternet);
			return HTTPResponse(0);
	...
	ProgressCallback*     progress) const
{
	...
	if (proxySettings.isSet())
	{
		...
		GW_DEBUG << LC << "Read proxy settings from options " << proxy_host << " " << proxy_port << std::endl;
	}
	...
	if (!proxy_host.empty())
	{
		...
		if (s_HTTP_DEBUG)
		{
			GW_NOTICE << LC << "Using proxy: " << proxy_addr << std::endl;
		}
		...
		if (!proxy_auth.empty())
		{
			if (s_HTTP_DEBUG)
			{
				GW_NOTICE << LC << "Using proxy authentication " << proxy_auth << std::endl;
			}
	...
	{
		GW_DEBUG << LC << "Removing proxy settings" << std::endl;
	...
	if (_simResponseCode < 0)
	{
		...
		if (!proxy_addr.empty())
		{
			...
			if (r != CURLE_OK)
			{
				GW_WARN << LC << "Proxy connect error: " << curl_easy_strerror(r) << std::endl;
				return HTTPResponse(0);
			}
		}
	...
	if (res != CURLE_ABORTED_BY_CALLBACK && res != CURLE_OPERATION_TIMEDOUT)
	{
		...
		{
			GW_DEBUG << LC << "detected multipart data; decoding..." << std::endl;
	...
	response._duration_s = GW_STOP_TIMER(get_duration);

	if (progress)
	{
		progress->stats()["http_get_time"] += GW_STOP_TIMER(http_get);
		progress->stats()["http_get_count"] += 1;
		if (response._cancelled)
			progress->stats()["http_cancel_count"] += 1;
	}
	...
	if (s_HTTP_DEBUG)
	{
...

		{
			...
			{
				GW_NOTICE << LC << "Average duration = " << s_HTTP_DEBUG_total_duration / (double)s_HTTP_DEBUG_request_count
					<< std::endl;
			}
		}
...
bool HTTPClient::doDownload(const std::string& url, const std::string& filename)
{
	initialize();
	...
	{
		GW_WARN << LC << "Error downloading file " << filename
...
namespace
{
	osgDB::ReaderWriter* getReader(const std::string& url, const HTTPResponse& response)
	{
		...
		{
			GW_WARN << LC << "Cannot find an OSG plugin to read response data (ext="
...
ReadResult HTTPClient::doReadImage(const HTTPRequest&    request,
	const osgDB::Options* options,
	...
	else
	{
		...
		if (HTTPClient::isRecoverable(result.code()))
		{
			if (callback)
			{
				if (s_HTTP_DEBUG)
				{
					GW_NOTICE << LC << "Error in HTTPClient for " << request.getURL() << " but it's recoverable" << std::endl;
				}
...
ReadResult HTTPClient::doReadNode(const HTTPRequest&    request,
	const osgDB::Options* options,
	ProgressCallback*     callback)
{
	initialize();
...
ReadResult HTTPClient::doReadObject(const HTTPRequest&    request,
	const osgDB::Options* options,
	ProgressCallback*     callback)
{
	initialize();
...
ReadResult HTTPClient::doReadString(const HTTPRequest&    request,
	const osgDB::Options* options,
	ProgressCallback*     callback)
{
	initialize();
