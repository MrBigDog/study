...
namespace
{
	...
	public:
		...
		void join()
		{
			...
			//_resPolygon.outer_boundary().is_convex()

			for (unsigned int i = 0; i < _resultPolygons.size(); ++i)
			{
			...
			for (it = _resPolygon.outer_boundary().vertices_begin();
				it != _resPolygon.outer_boundary().vertices_end(); ++it)
			{
				va->push_back(osg::Vec3d(CGAL::to_double(it->x()), CGAL::to_double(it->y()), 0.0));
			}
