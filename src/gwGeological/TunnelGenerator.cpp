...
namespace
{
	...
	//
	struct TrenchSegmentPara :public TunnelSegmentParaBase
	{
		TrenchSegmentPara(const osg::Plane& sPlane, const osg::Plane& ePlane,
			const osg::Vec3d& sPosition, const osg::Vec3d& ePosition, double vSize, double grade)
			: TunnelSegmentParaBase(sPlane, ePlane, sPosition, ePosition)
			, _vSize(vSize)
			, _grade(grade)
		{}
	...
	typedef std::vector<TrenchSegmentPara> TrenchSegmentParaVec;
		...
		double r, double angle, unsigned int segmentNum = 16)
	{
		...
		ColumnGeometryOptimizer cgo(center, frontDir, side, r/*halfWidth*/, false, segmentNum);
		arcGroup->accept(cgo);
	...
	osg::Node*generateTrenchGeometry()
	{
		return 0L;
	}
	...
	osg::Node * generateSquareTunnelSegment(osg::Node * geologicalGroup, const SquareTunnelSegmentPara & tunnelPara, bool isOnEarth)
	{
		...
		osg::Vec3d frontDir = normalize(endPos - startPos);
		osg::Vec3d worldUpDir = isOnEarth ? osg::Vec3d(0, 0, 1) : normalize((startPos + endPos)*0.5);
		osg::Vec3d sideDir = normalize(worldUpDir^frontDir);
		osg::Vec3d upDir = normalize(frontDir^sideDir);

		double halfWidth = width*0.5;
		double halfHeight = height*0.5;
		//double aveSize = (width + height)*0.5;
		...
		PlaneGeometryOptimizer pgo_l(startPos - sideDir*halfWidth, sideDir, frontDir/*, aveSize*/);
		leftProfileNode->accept(pgo_l);
		resultRoot->addChild(leftProfileNode/**/);

		Cutter rightCutter(sidePolytope);
		rightProfileNode->accept(rightCutter);
		PlaneGeometryOptimizer pgo_r(startPos + sideDir*halfWidth, -sideDir, frontDir/*, aveSize*/);
		rightProfileNode->accept(pgo_r);
		...
		PlaneGeometryOptimizer pgo_b(startPos - upDir*halfHeight, upDir, frontDir/*, aveSize*/);
		bottomProfileNode->accept(pgo_b);
		...
		else if (tunnelPara._topeType == SquareTunnelSegmentPara::PLANE_TOP)
		{
			...
			PlaneGeometryOptimizer pgo_u(startPos + upDir*halfHeight, -upDir, frontDir/*, aveSize*/);
			topProfileNode->accept(pgo_u);
	...
	//º¾¹µ;
	osg::Node* generateTrenchSegment(osg::Node * geologicalGroup, const TrenchSegmentPara & trenchPara, bool isOnEarth)
	{
		if (!geologicalGroup) return 0L;
		if (!trenchPara.isValid()) return 0L;

		osg::Vec3d startPos = trenchPara._startPostion;
		osg::Vec3d endPos = trenchPara._endPosition;
		double vsize = trenchPara._vSize;
		...
		osg::Plane startPlane(trenchPara._startPlane);
		osg::Plane endPlane(trenchPara._endPlane);

		double vgrade = osg::DegreesToRadians(trenchPara._grade);
		osg::Quat rrot(vgrade, frontDir);
		osg::Quat rot(-vgrade, frontDir);
		osg::Vec3d leftPlaneNormal = rot*worldUpDir;
		osg::Vec3d rightPlaneNormal = rrot*worldUpDir;

		osg::ref_ptr<osg::Group> resultRoot = new osg::Group;
		SliceOperator slicer(isOnEarth);

		osg::Plane leftPlane(leftPlaneNormal, startPos - sideDir*vsize - upDir*vsize);
		osg::Plane rightPlane(rightPlaneNormal, startPos + sideDir*vsize - upDir*vsize);
		osg::Plane bottomPlane(upDir, startPos - upDir*vsize);
		...
		Cutter leftCutter(sidePolytope);
		leftProfileNode->accept(leftCutter);
		PlaneGeometryOptimizer pgo_l(startPos - sideDir*vsize, sideDir, frontDir/*, aveSize*/);
		leftProfileNode->accept(pgo_l);
		resultRoot->addChild(leftProfileNode/**/);

		Cutter rightCutter(sidePolytope);
		rightProfileNode->accept(rightCutter);
		PlaneGeometryOptimizer pgo_r(startPos + sideDir*vsize, -sideDir, frontDir/*, aveSize*/);
		rightProfileNode->accept(pgo_r);
		resultRoot->addChild(rightProfileNode);
		...
		Cutter bottomCutter(updownPolytope);
		bottomProfileNode->accept(bottomCutter);
		PlaneGeometryOptimizer pgo_b(startPos - upDir*vsize, upDir, frontDir/*, aveSize*/);
		bottomProfileNode->accept(pgo_b);
		resultRoot->addChild(bottomProfileNode);
...
//º¾¹µ;
osg::Node* TrenchGenerator::operator()(osg::Node*geoGroup, const osg::Vec3dArray* va, double vsize, double grade)
{
	if (va->size() < 2 || !geoGroup) return 0L;
	...
	for (unsigned int i = 0; i < planeVector.size() - 1; ++i)
	{
		...
		TrenchSegmentPara para(sPlane, ePlane, va->at(i), va->at(i + 1), vsize, grade);
		if (para.isValid())
		{
			trenchs.push_back(para);
		}
	}
	osg::ref_ptr<osg::Group> trenchRoot = new osg::Group;
	for (TrenchSegmentParaVec::const_iterator it = trenchs.begin(); it != trenchs.end(); ++it)
	{
		osg::ref_ptr<osg::Node> tunnelSegNode = generateTrenchSegment(geoGroup, *it, _isOnEarth);
		if (tunnelSegNode)
		{
			trenchRoot->addChild(tunnelSegNode);
		}
	}
	trenchs.clear();
	trenchRoot->getOrCreateStateSet()->setMode(GL_NORMALIZE, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE | osg::StateAttribute::PROTECTED);

	setMaskState(trenchRoot);

	return trenchRoot.release();
}

//Éú³É²Ã¼ôÃæ;
void TrenchGenerator::generateSlicePlanes(const osg::Vec3dArray* va, double vsize, bool isOnEarth, SlicePLaneVector& out_slice_planes)
{
