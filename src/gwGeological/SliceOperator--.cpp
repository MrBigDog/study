...
namespace
{
	...
	void generateLoops(const Vec3dArrayVector &vaVector, DLoops& loops, Vec3dArrays& holes, const osg::Matrixd& mat)
	{
		...
		for (fit = arr.faces_begin(); fit != arr.faces_end(); ++fit)
		{
			...
			Polygon_2 polygon;
			getCbbArray<Ex_arrangement>(fit->outer_ccb(), polygon);
			loop->_polygons.push_back(polygon);
		...
		while (!isFinish)
		{
			...
			for (DLoops::const_iterator it = loops.begin(); it != loops.end(); ++it)
			{
				...
				for (hit = (*it)->_holes.begin(); hit != (*it)->_holes.end(); ++hit)
				{
					...
					{
						findHole = true;
						//for (unsigned int i = 0; i < (*it)->_polygons.size(); ++i)
						//{
							//if (isInHole(*tp, (*it)->_polygons[0]))
							//{
								tp = &((*it)->_polygons[0]);
								//tp = &((*it)->_va);
								//break;
	...
	osg::Node* generateProfileNode(const DLoops &loops, const Vec3dArrays& holes, osg::Geometry* geom, const osg::Plane &sliceplane, const osg::Matrixd& mat)
	{
		...
		for (DLoops::const_iterator it = loops.begin(); it != loops.end(); ++it)
		{
			...
			//是孔洞则忽略;
			bool ishole = true;
			for (unsigned int i = 0; i < loop->_polygons.size(); ++i)
			{
				if (!isHole(loops, loop->_polygons[i]))
				{
					ishole = false;
					break;;
				}
			...
			unsigned int vaSize = 0;
			for (PolygonVec::const_iterator pit = loop->_holes.begin(); pit != loop->_holes.end(); ++pit)
				vaSize += pit->size();
			...
			for (unsigned int i = 0; i < loop->_polygons.size(); ++i)
			{
				for (Polygon_2::Vertex_const_iterator vaIt = loop->_polygons[i].vertices_begin(); vaIt != loop->_polygons[i].vertices_end(); ++vaIt)
				{
	...
	static bool isHoleOf(const Polygon_2& p1, const Polygon_2& p2, const Vec3dArrayVector& edges, PointVec& pointOnEdge, PointVec& points, const osg::Matrixd& mat)
	{
		for (unsigned int i = 0; i < p1.size(); ++i)
		{
			...
			CGAL::Bounded_side bs = CGAL::bounded_side_2(p2.vertices_begin(), p2.vertices_end(), point, Kernel());// p2.bounded_side(point);
	...
	void reorganizeLoops(DLoops &loops, const Vec3dArrayVector& edges, const osg::Matrixd& mat)
	{
		for (DLoops::iterator it = loops.begin(); it != loops.end(); ++it)
		{
			...
			for (DLoops::const_iterator ait = loops.begin(); ait != loops.end(); ++ait)
			{
				...
				bool isholeof = false;
				for (unsigned int i = 0; i < loop1->_polygons.size(); ++i)
				{
					PointVec pointOnEdeg, otherPoints;
					if (isHoleOf(loop->_polygons[0], loop1->_polygons[i], edges, pointOnEdeg, otherPoints, mat))
					{
						isholeof = true;
						break;
					}
				...
				if (isholeof)
				{
					for (unsigned int i = 0; i < loop->_polygons.size(); ++i)
					{
						loop1->_polygons.push_back(loop->_polygons[i]);
						loop1->_holes.push_back(loop->_polygons[i]);
					}
					break;
				}
			}
...
{
	double x1 = loop1->_polygons[0].bbox().xmax() - loop1->_polygons[0].bbox().xmin();
	double y1 = loop1->_polygons[0].bbox().ymax() - loop1->_polygons[0].bbox().ymin();
	double x2 = loop2->_polygons[0].bbox().xmax() - loop2->_polygons[0].bbox().xmin();
	double y2 = loop2->_polygons[0].bbox().ymax() - loop2->_polygons[0].bbox().ymin();
	return x1*y1 < x2*y2;
}
