...
const bool fillRenderMode = false;
//
namespace
{
	...
	{
		switch (/*hole.bounded_side(pt)*/CGAL::bounded_side_2(hole.vertices_begin(), hole.vertices_end(), pt, Kernel()))
		{
	...
	osg::Geode* generateLine(osg::Vec3Array* va, const osg::Vec4& color, float width = 5.0f)
	{
		osg::ref_ptr<osg::Vec4Array> ca = new osg::Vec4Array;
		ca->push_back(color);
	...
	osg::Node* generateLine(const Vec3dArrayVector& vas, const osg::Vec4& color/*, const osg::Matrixd& mat*/, float width = 5.0f)
	{
		...
		osg::ref_ptr<osg::LineWidth> lw = new osg::LineWidth;
		lw->setWidth(5.0f);
		root->getOrCreateStateSet()->setAttributeAndModes(lw);
	...
	osg::Vec2Array* generateTextureCoord(const osg::Vec3d& normal, osg::ref_ptr<osg::Vec3Array> va)
	{
		...
		{
			double ts = plane_s.distance(*it)*0.02;
			double ty = plane_t.distance(*it)*0.02;
	...
	osg::Node* generateProfileNode(const DLoops &loops, const Vec3dArrays& holes, osg::Geometry* geom, const osg::Plane &sliceplane, const osg::Matrixd& mat)
	{
		...
		for (DLoops::const_iterator it = loops.begin(); it != loops.end(); ++it)
		{
			...
			if (!texture)
			{
				texture = dynamic_cast<osg::Texture2D*>(geom->getUserData());
				//osg::Image*image = dynamic_cast<osg::Image*>(geom->getUserData());
				//if (image)
				//{
			...
			if (texture)
			{
				osg::ref_ptr<osg::Vec2Array> ta = generateTextureCoord(sliceplane.getNormal(), va);
				profileGeom->setTexCoordArray(0, ta);
				profileGeom->getOrCreateStateSet()->setTextureAttributeAndModes(0, texture);
			}
	...
	static bool isHoleOf(const Polygon_2& p1, const Polygon_2& p2, const Vec3dArrayVector& edges, PointVec& pointOnEdge, PointVec& points, const osg::Matrixd& mat)
	{
		for (unsigned int i = 0; i < p1.size(); ++i)
		{
			...
			CGAL::Bounded_side bs = CGAL::bounded_side_2(p2.vertices_begin(), p2.vertices_end(), point, Kernel());// p2.bounded_side(p1.vertex(i));
...
//////////////////////////////////////////////////////////////////////////
SliceOperator::SliceOperator(bool isOnEarth, ModelType modelType) :_isOnEarth(isOnEarth), _modelType(modelType) {}
SliceOperator::~SliceOperator() {}

osg::Node * SliceOperator::slice(osg::Node * geoModel, const osg::Plane & slicePlane)
{
	if (_modelType == HULL)
	{
		return sliceNode(geoModel, slicePlane);
	}
	else if (_modelType == VOXEL)
	{
		return VolelMeshClipper::clipNode(geoModel, slicePlane);
	}
	return 0L;
}
...
osg::Node * SliceOperator::sliceGeom(osg::Geometry * geom, const osg::Plane & sliceplane)
{
	if (_modelType == HULL)
	{
		return sliceGeometry(geom, sliceplane);
	}
	else if (_modelType == VOXEL)
	{
		return VolelMeshClipper::clipGeom(geom, sliceplane);
	}
	return 0L;
}
...
osg::Node* SliceOperator::sliceGeometry(osg::Geometry* geom, const osg::Plane& sliceplane)
{
	if (!geom) return 0L;

	Vec3dArrayVector vaVector;	vaVector.clear();
	...
	//这个矩阵用于将三维平面映射到二维上，方便CGAL以polygon的方式处理;
	osg::Matrixd mat = osg::Matrixd::translate(transformVec)*osg::Matrixd::rotate(rot);
	osg::Matrixd inverseMat = osg::Matrixd::inverse(mat);

	bool isFault = false;
	if (geom->getUserValue("isFault", isFault))
	{
		if (isFault)//断层，非闭合区域，绘制线;
		{
			osg::ref_ptr<osg::Node> lineNode = generateLine(vaVector, getGeometryColor(geom), 3.0);
			return lineNode.release();
		}
	}
...
//osg::Node * SliceOperator::sliceVoxelGeom(osg::Geometry * voxelGeom, const osg::Plane & sliceplane)
//{
//	if (!voxelGeom) return 0L;
//
//	osg::ref_ptr<gwUtil::PlaneIntersector> intersector = new gwUtil::PlaneIntersector(sliceplane);
//	intersector->setRecordHeightsAsAttributes(false);
//
...
//	intersectionVisitor.reset();
//	intersectionVisitor.setIntersector(intersector.get());
//	voxelGeom->accept(intersectionVisitor);
//
//	gwUtil::PlaneIntersector::Intersections& intersections = intersector->getIntersections();
//	if (intersections.empty()) return 0L;
//
...
osg::Node* SliceOperator::sliceNode(osg::Node * geologicalModel, const osg::Plane& slicePlane)
{
