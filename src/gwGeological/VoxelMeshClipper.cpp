...
namespace
{
		...
		PolylineConnector<VC_FVF>& connector)
	{
		...
		float coeff1, coeff2;
		getlinePlaneCoefficient((*va)[ia], (*va)[ib], (*va)[ic], plane/*planeNormal, planePoint*/, coeff1, coeff2);
...
osg::Node* VolelMeshClipper::clipGeom(osg::Geometry* geom, const osg::Plane& plane/*const osg::Vec3d& planePoint, const osg::Vec3d& planeNormal*/)
{
	if (!geom) return 0L;
	...
	for (int i = 0; i < faceCount; ++i)
	{
		...
		else if (pointsOnPlane == 0)
		{
			...
			else if (pointsToLeft == 1)// One vertex to left
			{
				...
				intersectTrangle(va, ca, ia, ib, ic, plane/*planeNormal, planePoint*/, connector);
			}
...
namespace
{
	...
	public:
		...
		osg::Node* getClipResult() const { return _clipRoot.get(); }
...
osg::Node * VolelMeshClipper::clipNode(osg::Node * node, const osg::Plane& plane)
{
	if (!node) return 0L;

	GeomVisitor gv(plane);
	node->accept(gv);

	return gv.getClipResult();
}
